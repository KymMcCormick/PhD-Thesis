...
```{r}
Rank_dat <- din  %>% 
  group_by(memory,expectation) %>%
  summarise(Rank_1 = sum(R1_Corr),
            Rank_2 = sum(R2_Corr),
            Rank_3 = sum(R3_Corr),
            Rank_4 = sum(R4_Corr),
            Rank_5 = sum(R5_Corr),
...
Rank_N <- din %>%
  select(target ,memory, n, R1_Corr,R2_Corr, R3_Corr, R4_Corr, R5_Corr, R6_Corr, R7_Corr, R8_Corr)%>%
  filter(target =="P")
...
Rank_cum <- Rank_N %>% 
  summarise(Rank_1_C = sum(R1_Corr),
            Rank_1_F = sum(n)-sum(R1_Corr),
            Rank_2_C = sum(R2_Corr)+Rank_1_C,
            Rank_2_F = Rank_1_F + sum(n)-sum(R2_Corr),
            Rank_3_C = sum(R3_Corr)+Rank_2_C,
            Rank_3_F =  Rank_2_F + sum(n)-sum(R3_Corr),
            Rank_4_C = sum(R4_Corr)+Rank_3_C,
            Rank_4_F =  Rank_3_F + sum(n)-sum(R4_Corr),
            Rank_5_C = sum(R5_Corr)+Rank_4_C,
            Rank_5_F =  Rank_4_F + sum(n)-sum(R5_Corr),
            Rank_6_C = sum(R6_Corr)+Rank_5_C,
            Rank_6_F =  Rank_5_F + sum(n)-sum(R6_Corr),
            Rank_7_C = sum(R7_Corr)+Rank_6_C,
            Rank_7_F =  Rank_6_F + sum(n)-sum(R7_Corr),
            Rank_8_C =  sum(R8_Corr)+Rank_7_C,
            Rank_8_F =  Rank_7_F + sum(n)-sum(R8_Corr)
            ) 
...
Rank_dvector <- Rank_N %>% 
  group_by(memory) %>%
  summarise(Rank_1_C = sum(R1_Corr),
            Rank_2_C = sum(R2_Corr),
            Rank_3_C = sum(R3_Corr),
            Rank_4_C = sum(R4_Corr),
            Rank_5_C = sum(R5_Corr),
            Rank_6_C = sum(R6_Corr),
            Rank_7_C = sum(R7_Corr),
            Rank_8_C = sum(R8_Corr)
            )%>%
  select(-memory)%>%
  as.matrix() %>%
  t() %>%
...
```{r, echo=FALSE, include=FALSE}
Rank_prop_vector <- din %>% 
  summarise(Rank_1_CID = sum(R1_Corr)/sum(n),
            Rank_2_CID = sum(R2_Corr)/sum(n),
            Rank_3_CID = sum(R3_Corr)/sum(n),
            Rank_4_CID = sum(R4_Corr)/sum(n),
            Rank_5_CID = sum(R5_Corr)/sum(n),
            Rank_6_CID = sum(R6_Corr)/sum(n),
            Rank_7_CID = sum(R7_Corr)/sum(n),
            Rank_8_CID = sum(R8_Corr)/sum(n)
            ) %>% 
  as.matrix() %>% 
  t() %>% 
  as.vector
