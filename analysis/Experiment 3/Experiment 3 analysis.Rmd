...
TP <- sum(din$target == "P")
CID <- sum(din$CID)
FID <- sum(din$TAFoilID)
size <- 8
TP_Reject <- TP-sum(din$CID)
TA_Reject <- TA-(sum(din$TAFoilID)/size)
...
ID_dvector <- append(ID_dvector,TP_Reject,after= 0)
ID_dvector <- append(ID_dvector,TA_Reject,after = 11)
ID_dvector
...
ID_cum_vector <- din %>% 
  ...
  summarise(
    Miss = sum(Miss) ,
    TPFoilID = sum(TPFoilID),
    CID = sum(CID),
    CR = sum(CR) ,
    TAFoilID = sum(TAFoilID)/8*7,
    FID = sum(TAFoilID)/8
    ) 

ID_cum_vector <-  ID_cum_vector[10:1,]
ID_cum_vector1 <-  ID_cum_vector %>%
  transform(Miss = cumsum(Miss), TPFoilID = cumsum(TPFoilID), CID = cumsum(CID), CR = cumsum(CR),TAFoilID = cumsum(TAFoilID), FID = cumsum(FID)) %>%
  mutate(Miss = (TP - TPFoilID - CID), CR = TA-FID-TAFoilID) %>%
...
ID_cum_vector2 <-  ID_cum_vector %>%
  transform(Miss = cumsum(Miss), TPFoilID = cumsum(TPFoilID), CID = cumsum(CID), CR = cumsum(CR),TAFoilID = cumsum(TAFoilID), FID = cumsum(FID)) %>%
  mutate(Miss = (TP  - CID), CR = TA-FID) %>%
 select(-decile,-TPFoilID,-TAFoilID) %>%
