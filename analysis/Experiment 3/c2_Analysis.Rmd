...
data <- Experiment3data %>%
  ...
  separate(condition,c("memory","expectation","F68"))  %>%
...
data$CID <- if_else(data$F68 == "P" & data$Test_T1_suspectIdentified == "F68", 1, 0)
data$Miss <- if_else(data$F68 == "P" & data$Test_T1_suspectIdentified == "Silhouette", 1, 0)
data$CR <- if_else(data$F68 == "A" & data$Test_T1_suspectIdentified == "Silhouette", 1, 0)
data$TPFoilID <- if_else(data$F68 == "P" & data$Test_T1_suspectIdentified != "F68", 1, 0)
data$TAFoilID <- if_else(data$F68 == "A" & data$Test_T1_suspectIdentified != "Silhouette", 1, 0)
data$n <- if_else(!is.na(data$demographics_country) == "USA",1,0)
data$R1_Corr <- if_else(data$F68 == "P" & data$Rank_1 == "F68", 1, 0)
data$R2_Corr <- if_else(data$F68 == "P" & data$Rank_2 == "F68", 1, 0)
data$R3_Corr <- if_else(data$F68 == "P" & data$Rank_3 == "F68", 1, 0)
data$R4_Corr <- if_else(data$F68 == "P" & data$Rank_4 == "F68", 1, 0)
data$R5_Corr <- if_else(data$F68 == "P" & data$Rank_5 == "F68", 1, 0)
data$R6_Corr <- if_else(data$F68 == "P" & data$Rank_6 == "F68", 1, 0)
data$R7_Corr <- if_else(data$F68 == "P" & data$Rank_7 == "F68", 1, 0)
data$R8_Corr <- if_else(data$F68 == "P" & data$Rank_8 == "F68", 1, 0)
